...
class SEGSDetailerModelSwap:
    """
    SEGS detailer that takes direct model/clip/vae/positive/negative inputs
    instead of BASIC_PIPE, so any loaded diffusion model path can be wired in
    directly.
    ...
    def doit(
        ...
        if len(image) > 1:
            raise Exception(
                "[RemarkeeMax-Facedetailer] image batches are not supported. Use one image at a time."
            )
...
class RemarkeeMaxAdaptiveDenoise:
    """
...
NODE_CLASS_MAPPINGS = {
    "RemarkeeMax-Facedetailer": SEGSDetailerModelSwap,
    "SEGSDetailerModelSwap": SEGSDetailerModelSwap,  # generic alias used by the workflow
    "RemarkeeMax-AdaptiveDenoise": RemarkeeMaxAdaptiveDenoise,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "RemarkeeMax-Facedetailer": "Remarkee Max Facedetailer",
    "SEGSDetailerModelSwap": "Remarkee Max Facedetailer (Legacy)",
    "RemarkeeMax-AdaptiveDenoise": "Remarkee Max Adaptive Denoise",
}
