...
from ds_remint_v7 import apply_ds_remint_v8_2, is_ds_remint_v8_2
...
def handler(job):
    ...
    with tempfile.TemporaryDirectory(prefix=f"deepclean-{job_id}-") as tmpdir:
        ...
        try:
            ...
            if is_max_remint(expert_refinement):
            ...
            elif is_ds_remint_v8(expert_refinement):
                # DS ReMint V8 is the V7 terminal pipeline plus the V8 quality
                # floor (studio/high/balanced/strong) and the ghost re-life
                # ladder. Same wash -> camera re-life -> source-aware gate ->
                # single JPEG encode contract as V7.
                engine_report = apply_ds_remint_v8(
                    input_path=input_path,
                    output_path=cleaned_path,
                    creator_id=creator_id,
                    settings=expert_refinement,
            ...
            elif is_ds_remint_v8_2(expert_refinement):
                # DS ReMint V8.2 Max: degrade -> ghost launder at low res ->
                # neural restore (Real-ESRGAN via ComfyUI) -> ghost_lite
                # re-life -> single encode. Terminal like the other DS modes.
                engine_report = apply_ds_remint_v8_2(
                    input_path=input_path,
                    output_path=cleaned_path,
                    creator_id=creator_id,
                    settings=expert_refinement,
                ...
                cleaned_sha = sha256_file(cleaned_path)
                neural_texture_report = {"enabled": False, "reason": "integrated_into_ds_remint_v8_2"}
                content_repair_report = {"enabled": False, "reason": "integrated_into_ds_remint_v8_2"}
            elif is_ds_remint_v8_3(expert_refinement):
                # DS ReMint V8.3: the V8.2 degrade -> restore -> re-life
                # pipeline plus the wash-family unlock (qwen | zimage |
                # qwen+zimage). Terminal like the other DS modes.
                engine_report = apply_ds_remint_v8_3(
                    input_path=input_path,
                    output_path=cleaned_path,
                    creator_id=creator_id,
                    settings=expert_refinement,
                ...
                cleaned_sha = sha256_file(cleaned_path)
                neural_texture_report = {"enabled": False, "reason": "integrated_into_ds_remint_v8_3"}
                content_repair_report = {"enabled": False, "reason": "integrated_into_ds_remint_v8_3"}
            elif is_ds_remint_v8_8(expert_refinement):
                # DS ReMint V8.8 Coherent: wash -> single resample -> coherent
                # camera model (inverse ISP -> optics -> CFA -> MHC -> weak
                # ISP denoise -> forward ISP) -> one encode. Terminal.
                engine_report = apply_ds_remint_v8_8(
                    input_path=input_path,
                    output_path=cleaned_path,
                    creator_id=creator_id,
                    settings=expert_refinement,
                ...
                cleaned_sha = sha256_file(cleaned_path)
                neural_texture_report = {"enabled": False, "reason": "integrated_into_ds_remint_v8_8"}
                content_repair_report = {"enabled": False, "reason": "integrated_into_ds_remint_v8_8"}
            elif is_ds_remint_v8_9(expert_refinement):
                # DS ReMint V8.9: V8.8 coherent pipeline with data-driven
                # defaults (Qwen wash, deep degrade 0.75) + baseline routing.
...
def final_naturalization_config(cfg, expert_refinement):
    if (
        ...
        or is_ds_remint_v8_2(expert_refinement)
