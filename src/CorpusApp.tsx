    ...
    <div className="corpus">
      ...
      <div className="cp-grid">
        ...
        <section className="cp-card cp-sets-card">
          ...
          <div className="cp-form-row"><input value={detectorModel} onChange={(event) => setDetectorModel(event.target.value)} placeholder="Detector model" /><input value={detectorVersion} onChange={(event) => setDetectorVersion(event.target.value)} placeholder="Detector version (optional)" /></div>
          <button className="cp-btn cp-primary" disabled={!selectedSet?.locked_at || !engineRelease.trim() || !!busy} onClick={() => void runAction("experiment", () => createCorpusExperiment({ corpusSetId: selectedSet!.id, engineRelease: engineRelease.trim(), detectorVendor: "g1", detectorMode: "real", detectorModel: detectorModel.trim() || undefined, detectorVersion: detectorVersion.trim() || undefined, configSet: ["A", "1A", "2B"] }), "Comparable experiment created.")}><FlaskConical size={13} /> Create experiment</button>
